        ...
        <div className="px-6 py-5 border-t border-border/50 flex flex-col md:flex-row items-center justify-between gap-6 bg-muted/10">
            <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="flex flex-col order-2 sm:order-1">
                    ...
                    <span className="text-[11px] font-medium text-foreground whitespace-nowrap">
                        <span className="text-primary font-semibold">{displayedCount > 0 ? ((currentPage - 1) * pageSize) + 1 : 0}-{Math.min(currentPage * pageSize, totalRecords)}</span> de <span className="font-semibold">{totalRecords}</span> registros
                    </span>
                </div>
                <div className="relative order-1 sm:order-2">
                    <select
                        ...
                        className="pl-3 pr-8 py-2 bg-card border border-border/50 text-foreground text-[10px] rounded-xl focus:ring-4 focus:ring-primary/10 block font-semibold outline-none tracking-[0.08em] appearance-none shadow-sm cursor-pointer"
                    >
            ...
            <div className="flex items-center justify-center gap-2 w-full md:w-auto">
                ...
                <div className="px-5 py-2 bg-card border border-border/50 rounded-xl min-w-[100px] text-center shadow-sm">
                    <p className="text-[8px] font-semibold text-muted-foreground/40 uppercase tracking-[0.08em] leading-none mb-0.5">Página</p>
                    <p className="text-[11px] font-semibold text-foreground leading-tight">{currentPage} / {totalPages}</p>
                </div>
                <Button
                    variant="outline"
                    size="icon"
