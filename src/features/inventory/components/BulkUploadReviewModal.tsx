        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
                    <div>
                        <h3 className="text-lg font-bold text-slate-800">Revisión de Carga de Productos</h3>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {fileAnalysis.rows.map((row, index) => (
                                    <tr key={index} className={`hover:bg-slate-50 transition-colors ${!row.isValid ? 'bg-red-50/30' : ''}`}>
                                        ...
                                        <td className={`px-4 py-3 font-medium ${!row.data.CodigoInterno ? 'text-red-500 italic' : 'text-slate-700'}`}>
                                            {row.data.CodigoInterno || '—'}
                                        </td>
                                        <td className={`px-4 py-3 ${!row.data.NombreProducto ? 'text-red-500 italic' : 'text-slate-700'}`}>
                                            {row.data.NombreProducto || '[Vacío]'}
                                        </td>
                                        <td className={`px-4 py-3 ${!row.data.CodigoCategoria ? 'text-red-500 italic' : 'text-slate-600'}`}>
