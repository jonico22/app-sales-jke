        ...
        <div className="min-h-full bg-background">
            ...
            <div className="border-b border-border bg-card">
                <div className="max-w-7xl mx-auto px-6 py-5 space-y-4">
                    <button
                        ...
                        className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground hover:text-primary transition-colors group"
                    >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-black text-foreground tracking-tight leading-none">Nueva Operación de Traslado</h1>
                                <p className="text-muted-foreground text-sm mt-1 font-medium">
                                    Configura los parámetros para el envío de stock entre sucursales.
                                </p>
                            </div>
                        </div>
                        <Badge className="bg-amber-500/10 text-amber-600 border border-amber-500/30 px-3 py-1.5 font-black text-[10px] uppercase tracking-widest w-fit">
                            ● Borrador
                        </Badge>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
                ...
                <div className="flex justify-center">
                    <div className="bg-muted/40 p-1.5 rounded-2xl border border-border/60 flex gap-1">
                        <button
                            onClick={() => setActiveTab('total')}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all duration-200 ${
                                activeTab === 'total'
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card className="lg:col-span-2 rounded-2xl border-border shadow-sm bg-card">
                        <div className="h-1 w-full bg-gradient-to-r from-primary/60 via-primary to-primary/60 rounded-t-2xl" />
                        <div className="p-6 space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="font-black text-sm text-foreground uppercase tracking-wider">Configuración del Traslado</h2>
                                    <p className="text-[10px] text-muted-foreground font-medium mt-0.5">Define el origen y destino del stock</p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col md:flex-row md:items-end gap-4">
                                ...
                                <div className="space-y-2 flex-1">
                                    ...
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button
                                                variant="outline"
                                                className={`w-full h-12 px-4 rounded-xl text-[11px] font-bold uppercase tracking-wider flex items-center justify-between gap-3 transition-all ${
                                                    originBranchId
                                        ...
                                        <DropdownMenuContent className="w-[240px] bg-card border-border shadow-2xl rounded-xl p-1.5 max-h-[300px] overflow-y-auto">
                                            {branches.map(branch => (
                                                <DropdownMenuItem
                                                    key={branch.id}
                                                    className="py-2.5 px-3 rounded-lg text-[11px] font-bold uppercase tracking-wider cursor-pointer hover:bg-muted data-[highlighted]:bg-primary/5 data-[highlighted]:text-primary"
                                ...
                                {/* Arrow */}
                                <div className="hidden md:flex items-end justify-center pb-1 shrink-0">
                    ...
                    <Card className="rounded-2xl border-border shadow-sm bg-card">
                        <div className="h-1 w-full bg-gradient-to-r from-sky-400/60 via-sky-500 to-sky-400/60 rounded-t-2xl" />
                        <div className="p-6 space-y-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="font-black text-sm text-foreground uppercase tracking-wider">Identificación</h2>
                                    <p className="text-[10px] text-muted-foreground font-medium mt-0.5">Referencia y notas de la operación</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        className="h-11 bg-muted/20 border-border rounded-xl text-[11px] font-black text-primary uppercase tracking-widest focus:ring-2 focus:ring-primary/20"
                                    />
                                </div>
                                {activeTab === 'total' && (
                                    <div className="space-y-1.5 animate-in fade-in duration-300">
                                        ...
                                        <Textarea
                                            placeholder="Escribe el motivo o detalles del traslado..."
                                            rows={4}
                                            value={notes}
                ...
                {activeTab === 'selection' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
                        ...
                        <div className="p-6 border-t border-border/40 flex items-center justify-between">
                            <span className="text-[10px] font-bold text-muted-foreground uppercase opacity-50">
                                Mostrando {products.length > 0 ? (currentPage - 1) * 10 + 1 : 0}-{Math.min(currentPage * 10, totalProducts)} de {totalProducts} productos
                            </span>
                            <div className="flex items-center gap-2">
                                ...
                                {[...Array(totalPages)].map((_, i) => (
                                    <Button
                                        ...
                                        className={`h-8 w-8 p-0 rounded-lg text-[10px] font-black ${currentPage === i + 1 ? "bg-primary text-primary-foreground" : "border-border"}`}
                                    >
                                        {i + 1}
                                    </Button>
                    ...
                    <div className="fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-md border-t border-border p-4 z-40 flex items-center justify-between">
                         <div className="flex items-center gap-4">
                            <Badge className="bg-primary/10 text-primary border-primary/20 px-3 py-1 font-black text-[10px] uppercase tracking-widest">
                                {selectedItems.filter(i => i.quantity > 0).length} Ítems Seleccionados
                            </Badge>
                            <span className="text-[10px] font-black uppercase text-muted-foreground opacity-40">|</span>
                            <button 
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <Button
                                ...
                                className="h-14 px-8 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground shadow-2xl shadow-primary/30 flex items-center gap-3 text-xs font-black uppercase tracking-[0.15em] transition-all hover:scale-[1.02] active:scale-[0.98]"
                            >
                                {isLoading ? (
                                    <Loader2 className="w-5 h-5 animate-spin" />
                                ) : (
